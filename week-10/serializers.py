...
# ─────────────────────────────────────────────────────────────────────────────
# ProductImage serializer  (Week 10)
# ─────────────────────────────────────────────────────────────────────────────

def serialize_product_image(image):
    """Serialize ProductImage model to dict"""
    return {
        'id':            image.id,
...
def serialize_product_images(images):
    """Serialize list of ProductImage models"""
    return [serialize_product_image(img) for img in images]


# ─────────────────────────────────────────────────────────────────────────────
# Product serializer  (Week 10: includes images)
# ─────────────────────────────────────────────────────────────────────────────

def serialize_product(product, include_images=True):
    """
    Serialize Product model to dict.
    Pass include_images=False in list views to avoid N+1 queries.
    """
    data = {
        'id':            product.id,
        'name':          product.name,
    ...
    if include_images:
        imgs = sorted(
            product.images,
            key=lambda i: (not i.is_primary, i.created_at)
        )
        data['images'] = serialize_product_images(imgs)
        # Convenience shortcut: primary image thumbnail (or first if none set)
        primary = next((i for i in imgs if i.is_primary), imgs[0] if imgs else None)
        data['primary_image_url']     = primary.image_url     if primary else None
        data['primary_thumbnail_url'] = primary.thumbnail_url if primary else None
...
def serialize_products(products):
    ...
    Images are omitted in list view for performance.
    """
    return [serialize_product(product, include_images=False) for product in products]
