...
with app.app_context():
    ...
    if app.config.get("STORAGE_BACKEND") == "local":
        # Ensure upload directories exist on startup for local storage mode.
...
def serve_upload(filename):
    ...
    if app.config.get("STORAGE_BACKEND") != "local":
        return jsonify({
            "error": "Media is not served from local disk",
            "hint": "Use the image_url / thumbnail_url fields returned by the API"
        }), 404
...
def health_check():
    return jsonify({
        'status': 'ok',
        'message': 'Week 12 Deployment API',
        'version': '3.0.0',
        ...
        'features': {
            ...
            'storage':       f"Media backend: {app.config.get('STORAGE_BACKEND', 'local')}",
            'deployment':    'Render/Heroku/AWS compatible with environment-based config',
        }
    }), 200
